...
1. **命令行**（一次性抓某个/某些竞品）：`node scrape.js <链接>`，见下文。
2. **网页看板**（持续监控、可视化、变更追踪）：双击 `看板.bat`（或 `npm run web`），
   浏览器打开 `http://localhost:8787`：
   - 添加竞品（贴链接）、一键「抓取全部」；
   - 列表页竞品卡片 + **有更新** 标记；
   - 点进详情页：按 **五图 / 置顶大图 / icon / 视频 / 活动图** 筛选；
   - **变更记录**时间线：每次对方换图都留痕（旧→新对比，历史从开始监控起累积）。

   数据存在本地 `data/`（不入库）。代理端口由启动器自动从系统代理读取。
